...
public class DrawFractal implements ActionListener{
  ...
  /**
   * This listens for when a button is clicked on the window.
   * 
   * @param  e  what is called when a button is clicked
   */
  public void actionPerformed(ActionEvent e){
    JButton temp = (JButton) e.getSource();
    String buttonText = temp.getText();
    if(buttonText.equals("Snowflake")){
      Triangle triangle = new Triangle(new Point(400, 400), 400);                                                  //stores the base triangle for the snowflake
      SnowflakePicture snowflake = new SnowflakePicture(new Triangle[]{triangle});                                 //stores the snowflake
      snowflake.draw(4, getGraphics());
    }
    if(buttonText.equals("BoxFlake")){
      Rectangle square = new Rectangle(new Point(400, 400), 400);                                                  //stores the base square for the boxflake
      SnowflakePicture boxflake = new SnowflakePicture(new Rectangle[]{square});                                   //stores the boxflake
      boxflake.draw(4, getGraphics());
    }
    if(buttonText.equals("Tree")){
      Line line = new Line(new Point(0, 800), new Point(300, 500));                                                //stores the base line for the tree
      TreePicture treePic = new TreePicture(new Line[]{line});                                                     //stores the tree
      treePic.draw(4, getGraphics());
    }
    if(buttonText.equals("Triangle Fractal")){
      Triangle triangle1 = new Triangle(new Point(400,400), 400);                                                  //stores one of the triangles for for triangle picture
      Triangle triangle2 = new Triangle(new Point(200, 200), 200);                                                 //stores one of the triangles for for triangle picture
      Triangle triangle3 = new Triangle(new Point(600, 600), 200);                                                 //stores one of the triangles for for triangle picture
      TrianglePicture trianglePic = new TrianglePicture(new Triangle[]{triangle1, triangle2, triangle3});          //stores the triangle picture
      trianglePic.draw(4, getGraphics());
    }
    if(buttonText.equals("Carpet")){
      Rectangle square = new Rectangle(new Point(400, 400), 400);                                                  //stores the base square for the carpet
      CarpetPicture carpet = new CarpetPicture(new Rectangle[]{square});                                           //stores the carpet
